...
class entity():
    ...
    def intro_dev(self):
        print(
            self.name + self.role + self.hp + 
            self.atk + self.mp + self.df + self.description
            )
    # introducing 
    def intro(self):
...
generate_character()
name1, role1, hp1, atk1, mp1, df1, dscpt1 = generate_character().split(",", 6)
entity1 = entity(name1, role1, int(hp1), int(atk1), int(mp1), int(df1), dscpt1)
entity1.intro_dev()

name2, role2, hp2, atk2, mp2, df2, dscpt2 = generate_character().split(",", 6)
